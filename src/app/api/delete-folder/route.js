...
export async function POST(request) {
  ...
  try {
    ...
    const objectsToDelete = listData.Contents ? listData.Contents.map(item => ({ Key: item.Key })) : [];

    if (objectsToDelete.length === 0) {
      return new Response('No objects found in the specified folder.', { status: 404 });
    }
    ...
    return new Response(`Folder ${folderName} deleted successfully at path ${userName}/${folderName}`, { status: 200 });
  } catch (error) {
    console.error(error);
